...
class Dataset(BaseDataset):
    ...
    def __getitem__(self, i):
        ...
        if self.augmentation:
            ...
            # after resize
            check_set = np.array([0, 1, 2])
            result = np.in1d(mask_remap, check_set)
            all_in_set = np.all(result)
            assert all_in_set
        image = image.transpose(2, 0, 1)

        # count_ones = np.count_nonzero(mask_remap == 2)
        # print(count_ones)
...
model = CamVidModel("FPN", "resnet18", in_channels=3, out_classes=OUT_CLASSES)
